...
def retrain():
	...
	load = False
	if load:
		with open("live_model_k.model", "rb") as fp:   #Pickling
			clf = pickle.load(fp)
		with open("live_scaler_k.model", "rb") as fp:   #Pickling
		...
		clf.fit(X, y)
		with open("live_model_k_nodwt.model", "wb") as fp1:   #Pickling
			pickle.dump(clf, fp1)
		with open("live_scaler_k_nodwt.model", "wb") as fp1:   #Pickling
